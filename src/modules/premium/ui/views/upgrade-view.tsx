            ...
            <div className="mt-4 flex-1 flex flex-col gap-y-10 items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {products.map((product) => {
                        const isHighlighted =
                            String((product.metadata as any)?.variant ?? "").toLowerCase() === "highlighted" ||
                            (product.metadata as any)?.highlighted === true ||
                            (product.metadata as any)?.highlighted === "true" ||
                            product.prices[0]?.recurringInterval?.includes("year");
